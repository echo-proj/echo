...
public class DocumentVersionService {
  ...
  public VersionResponse createVersion(UUID userId, UUID documentId, CreateVersionRequest request) {
    ...
    DocumentContent content =
        contentRepository
            ...
            .orElseThrow(() -> new BadRequestException("Document has no content to version"));

    long versionCount = versionRepository.countByDocumentId(documentId);
    if (versionCount >= MAX_VERSIONS_PER_DOCUMENT) {
      deleteOldestVersion(documentId);
    }

    Integer nextVersionNumber =
        versionRepository.findMaxVersionNumberByDocumentId(documentId).map(max -> max + 1).orElse(1);

    User createdBy = userRepository.findById(userId).orElseThrow();

    DocumentVersion version =
        new DocumentVersion(document, nextVersionNumber, content.getState(), createdBy, request.getLabel());
  ...
  public void deleteVersion(UUID userId, UUID documentId, UUID versionId) {
    ...
    if (!DocumentAccessControl.isOwner(userId, document.getOwner().getId())) {
      throw new BadRequestException("Only the owner can delete versions");
    }
  ...
  private void deleteOldestVersion(UUID documentId) {
    ...
    if (!versions.isEmpty()) {
      versionRepository.delete(versions.get(0));
    }
  }
